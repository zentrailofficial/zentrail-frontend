  ...
  return (
    <div className="bg-[#DEF2FC]">
      <div className="custom-container py-6 md:py-10">
        <div className="flex justify-between flex-wrap gap-3 items-center">
          ...
          <div className="flex  gap-5">
            <button
              aria-label="trek"
              onClick={() => filterTour("")}
              className={`flex gap-2 items-center cursor-pointer dm_sans hover:text-black ${selected == "" ? "text-black" : "text-[#4D5875]"
                }`}
            >
              <Image
            ...
            <button
              ...
              className={`flex gap-2 items-center cursor-pointer dm_sans hover:text-black ${selected == "tour" ? "text-black" : "text-[#4D5875]"
                }`}
            >
              <Image
            ...
            <button
              ...
              className={`flex gap-2 items-center cursor-pointer dm_sans hover:text-black ${selected == "trek" ? "text-black" : "text-[#4D5875]"
                }`}
            >
              <Image
                src="/homepage/upcommingicon2.svg"
        ...
        {loading ? <div className="h-[300px] w-full col-span-2 flex justify-center items-center">
          <MiniLoader />
        </div>
          : <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] lg:gap-x-[110px] gap-y-15 mt-15 mb-8 pl-[20px]">
            {data?.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="absolute size-[120px] sm:size-[120px] md:size-[160px] top-[-10px] md:top-[-20px] left-[-20px] shrink-0">
                      ...
                      {/* gradient overlay */}
                      <div className=" rounded-full absolute  inset-0 bg-gradient-to-t  from-black/80  via-black/40 to-transparent opacity-0 
                         transition-opacity  duration-500 hover:opacity-80  focus:opacity-80  active:opacity-80  group-hover:opacity-80 
                              group-focus:opacity-80  group-active:opacity-80"></div>
                    </div>
                    <div className="ml-[90px] md:ml-[140px]">
                      <p
                        className="dm_sans ms-1 text-[14px] md:text-[18px] line-clamp-1 font-medium mb-2 leading-[14px] md:leading-[18px]"
                      >
                        {val.title}
                      </p>
                      ...
                      <div>
                        ...
                        <p className="dm_sans text-[12px] md:text-[14px]  text-[#4D5875] mt-1 line-clamp-1">
                          <PinDropIcon className="text-blue-600" />{" "}
                          {val.state}
                        </p>
                      </div>
                      {/* <div
              ...
              <div className="h-[300px] w-full col-span-2 flex justify-center items-center">
                <p className="text-red-600 dm_sans">{"No trail/tours/treks available here right now! We’ll be adding exciting tours soon — meanwhile, explore our upcoming trips."}</p>
              </div>
            )}
          </div>}
        {upcommingTrips?.length == 4 && !loading && <div className="flex justify-center col-span-2">
